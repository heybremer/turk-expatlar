  ...
  return (
    <div className={cn("flex flex-wrap items-center gap-2", className)}>
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="flex items-center gap-1.5 rounded-lg bg-background px-3 py-1.5 text-xs font-medium text-text transition-colors hover:bg-border"
      ...
      >
        {copied ? (
          <><Check className="h-3.5 w-3.5 text-success" /> Kopyalandı</>
        ) : (
          <><Copy className="h-3.5 w-3.5" /> Kopyala</>
        )}
      </button>

      {/* Native share (sadece desteklenen cihazlarda göster) */}
      {typeof window !== "undefined" && "share" in navigator && (
        <button
          ...
          aria-label="Daha fazla paylaşım seçeneği"
        >
